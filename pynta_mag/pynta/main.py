...
class Pynta:
    def __init__(self,path,rxns_file,surface_type,metal,label,launchpad_path=None,fworker_path=None,
        vacuum=8.0,repeats=(3,3,4),slab_path=None,software="Espresso", pbc=(True,True,False),socket=False,queue=False,njobs_queue=0,a=None,starting_magnetization=None,  # Azeez for magnetic added starting_magnetization=None
        ...
        ### ====== Azeez for mag START ========= #####
        # Store magnetization settings
        self.starting_magnetization = starting_magnetization or {}
        
        # Only set nspin=2 in software_kwargs for Espresso
        if self.software.lower() == "espresso" and self.starting_magnetization:
            self.software_kwargs['nspin'] = 2
            
            # Process gas phase kwargs
            if software_kwargs_gas:
                self.software_kwargs_gas = software_kwargs_gas
            else:
                self.software_kwargs_gas = deepcopy(self.software_kwargs)
            ...
            self.software_kwargs_gas['nspin'] = 2
            
            # Update TS kwargs
            self.software_kwargs_TS = deepcopy(self.software_kwargs)
            if TS_opt_software_kwargs:
                for key, val in TS_opt_software_kwargs.items():
                    self.software_kwargs_TS[key] = val
            self.software_kwargs_TS['nspin'] = 2
            
            # Update lattice optimization kwargs
            self.lattice_opt_software_kwargs = deepcopy(self.software_kwargs)
            if lattice_opt_software_kwargs:
                for key, val in lattice_opt_software_kwargs.items():
                    self.lattice_opt_software_kwargs[key] = val
            self.lattice_opt_software_kwargs['nspin'] = 2
            ### ====== Azeez for mag STOP ========= #####
    ...
    def generate_slab(self,skip_launch=False):
        ...
        if self.a is None:
            a = get_lattice_parameters(self.metal,self.surface_type,self.software,self.lattice_opt_software_kwargs,magnetization_dict=self.starting_magnetization) ##Azeez added magnetization_dict=self.starting_magnetization
        ...
        #### ========= Azeez for mag START ====== ######
        # Set magnetic moments on slab
        if self.starting_magnetization:
            slab = set_initial_magnetic_moments_by_element(slab, self.starting_magnetization)
        ##### ======= Azeez for mag STOP ======== ######
        slab.pbc = self.pbc
        write(os.path.join(self.path,"slab_init.xyz"),slab)
        self.slab_path = os.path.join(self.path,"slab.xyz")
        if self.software != "XTB":
            fwslab = optimize_firework(os.path.join(self.path,"slab_init.xyz"),self.software,"slab",magnetization_dict=self.starting_magnetization, ### Azeez added this: magnetization_dict=self.starting_magnetization,
    ...
    def setup_adsorbates(self):
        ...
        for sm,mol in self.mol_dict.items():
            ...
            adest_task = MolecularAdsorbateEstimate({"mol": mol.to_adjacency_list(),"mol_name": sm, "slab_path": self.slab_path, "magnetization_dict": self.starting_magnetization, #### Azeez added this "magnetization_dict": self.starting_magnetization,
    ...
    def setup_transition_states(self,adsorbates_finished=False):
        ...
        # if irc_mode = "skip" : do not conduct IRC
            ...
            os.makedirs(ts_path)
            ts_task = MolecularTSEstimate({"rxn": rxn,"ts_path": ts_path,"slab_path": self.slab_path,"adsorbates_path": os.path.join(self.path,"Adsorbates"), "magnetization_dict": self.starting_magnetization, #### Azeez added this "magnetization_dict": self.starting_magnetization,
