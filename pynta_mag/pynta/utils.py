...
def set_initial_magnetic_moments_by_element(atoms, magnetization_dict):
    """
    Set initial magnetic moments on atoms based on element type.
    
    Parameters:
    -----------
    atoms : ase.Atoms
        The atoms object to set magnetic moments on
    magnetization_dict : dict
        Dictionary mapping element symbols to magnetic moments
        e.g., {'Fe': 2.2, 'O': 0.8, 'H': 0.0}
    
    Returns:
    --------
    atoms : ase.Atoms
        The atoms object with magnetic moments set
    """
    if magnetization_dict:
        magmoms = [magnetization_dict.get(atom.symbol, 0.0) for atom in atoms]
