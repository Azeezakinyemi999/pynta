from pynta.utils import set_initial_magnetic_moments_by_element #Azeez added this for starting magnetization
...
def optimize_firework(xyz,software,label,opt_method=None,sella=None,socket=False,order=0,software_kwargs={},opt_kwargs={},magnetization_dict=None, ###Azeez added this : magnetization_dict=None,
    ...
    if magnetization_dict: d["magnetization_dict"] = magnetization_dict  # Azeez added this if magnetization_dict: d["magnetization_dict"] = magnetization_dict  
...
class MolecularOptimizationTask(OptimizationTask):
    ...
    optional_params = ["software_kwargs","opt_method", "magnetization_dict", 
        "opt_kwargs","run_kwargs", "constraints","sella","order","socket","time_limit_hrs","fmaxhard","ignore_errors","target_site_num","metal","facet"] ##Azeez added "magnetization_dict", 
    def run_task(self, fw_spec):
        errors = []
        software_kwargs = deepcopy(self["software_kwargs"]) if "software_kwargs" in self.keys() else dict()
        magnetization_dict = self.get("magnetization_dict", None) ## Azeez added this magnetization_dict = self.get("magnetization_dict", None) 
        ...
        try:
            if suffix == "xyz":
                ...
                sp = read(xyz)
            ###======= Azeez for mag START =========#######
            # Set magnetic moments before assigning calculator
            if magnetization_dict and self["software"].lower() == "espresso":
                sp = set_initial_magnetic_moments_by_element(sp, magnetization_dict)
            ###======= Azeez for mag STOP =========#######
...
class MolecularOptimizationFailTask(OptimizationTask):
    required_params = ["software","label"]
    optional_params = ["software_kwargs","opt_method", "magnetization_dict",
        "opt_kwargs","run_kwargs"]      ### Azeez added "magnetization_dict",
    def run_task(self, fw_spec):
        print(fw_spec)
        software_kwargs = deepcopy(self["software_kwargs"]) if "software_kwargs" in self.keys() else dict()
        software = name_to_ase_software(self["software"])(**software_kwargs)
...
def energy_firework(xyz,software,label,software_kwargs={},magnetization_dict=None, parents=[],out_path=None,ignore_errors=False):   #Azeez added  magnetization_dict=None,
    d = {"xyz" : xyz, "software" : software, "label" : label}
    if software_kwargs: d["software_kwargs"] = software_kwargs
    if magnetization_dict: d["magnetization_dict"] = magnetization_dict ### Azeez added this if magnetization_dict: d["magnetization_dict"] = magnetization_dict 
    d["ignore_errors"] = ignore_errors
...
class MolecularEnergyTask(EnergyTask):
    required_params = ["software","label"]
    optional_params = ["software_kwargs","energy_kwargs","ignore_errors", "magnetization_dict"] ##Azeez added "magnetization_dict"
    def run_task(self, fw_spec):
        xyz = self['xyz']
        ...
        try:
            sp = read(xyz)
            # Set magnetic moments before assigning calculator
            if magnetization_dict and self["software"].lower() == "espresso":       # Azeez added this:  if magnetization_dict and self["software"].lower() == "espresso":
                sp = set_initial_magnetic_moments_by_element(sp, magnetization_dict)    # Azeez added this: sp = set_initial_magnetic_moments_by_element(sp, magnetization_dict) 
...
def vibrations_firework(xyz,software,label,software_kwargs={},parents=[],out_path=None,constraints=[], magnetization_dict=None, socket=False,ignore_errors=False): ### Azeez added magnetization_dict=None,
    d = {"xyz" : xyz, "software" : software, "label" : label, "socket": socket}
    if software_kwargs: d["software_kwargs"] = software_kwargs
    if magnetization_dict: d["magnetization_dict"] = magnetization_dict ##Azeez added if magnetization_dict: d["magnetization_dict"] = magnetization_dict
    if constraints: d["constraints"] = constraints
    d["ignore_errors"] = ignore_errors
...
class MolecularVibrationsTask(VibrationTask):
    required_params = ["software","label"]
    optional_params = ["software_kwargs","constraints","ignore_errors","socket", "magnetization_dict"] ##Azeez Added "magnetization_dict"
    def run_task(self, fw_spec):
        indices = None
        xyz = self['xyz']
        label = self["label"]
        ...
        try:
            ...
            # Set magnetic moments before assigning calculator
            if magnetization_dict and self["software"].lower() == "espresso":       #Azeez added this 
                sp = set_initial_magnetic_moments_by_element(sp, magnetization_dict)      #Azeez added this
...
class MolecularAdsorbateEstimate(FiretaskBase):
    required_params = ["mol","mol_name","slab_path","path","metal","facet","sites","site_adjacency",
                        ...
                        "fmaxopt","socket"]
    optional_params = ["out_path","spawn_jobs","magnetization_dict", "nprocs","postprocess"] ## Azeez added "magnetization_dict",
    def run_task(self, fw_spec):
        ...
        magnetization_dict = self.get("magnetization_dict", None) ##Azeez added this
        ...
        if spawn_jobs:
            ...
            for xyz in xyzs:
                ...
                fwopt = optimize_firework(os.path.join(path,"Adsorbates",mol_name,prefix,prefix+"_init.xyz"),
                    ...
                    magnetization_dict=magnetization_dict, ##Azeez added this magnetization_dict=magnetization_dict,
...
class MolecularTSEstimate(FiretaskBase):
    ...
    optional_params = ["out_path","spawn_jobs","nprocs","IRC_obj_dict","magnetization_dict", "postprocess"] ##Azeez added "magnetization_dict",
    def run_task(self, fw_spec):
        ...
        for Eind in Einds:
            if Es[Eind]/Emin < Eharmtol: #include all TSs with energy close to Emin
                ...
                xyzsout.append(xyzs[Eind])
        # Modify the obj_dicts to include magnetization     ##Azeez added this 
        if magnetization_dict:          ##Azeez added this
            # These dictionaries are already retrieved from self at the beginning           ##Azeez added this
            # We need to modify them to include magnetization       ##Azeez added this
            opt_obj_dict = deepcopy(self["opt_obj_dict"])       ##Azeez added this
            vib_obj_dict = deepcopy(self["vib_obj_dict"])       ##Azeez added this
            
            opt_obj_dict["magnetization_dict"] = magnetization_dict ##Azeez added this
            vib_obj_dict["magnetization_dict"] = magnetization_dict ##Azeez added this
            
            if IRC_obj_dict:        ##Azeez added this
                IRC_obj_dict = deepcopy(IRC_obj_dict)       ##Azeez added this
                IRC_obj_dict["magnetization_dict"] = magnetization_dict     ##Azeez added this
        else:       ##Azeez added this
            # Use the original dicts if no magnetization    ##Azeez added this
            opt_obj_dict = self["opt_obj_dict"]     ##Azeez added this
            vib_obj_dict = self["vib_obj_dict"]     ##Azeez added this
        if spawn_jobs:          
...
class MolecularCollect(CollectTask):
    ...
    optional_params = ["detour","postprocess","metal","facet","sites", "magnetization_dict","site_adjacency","slab_path"]
    def run_task(self, fw_spec):
...
class MolecularIRC(FiretaskBase):
    ...
    optional_params = ["software","socket",
            "software_kwargs", "opt_kwargs", "run_kwargs", "constraints", "ignore_errors", "forward" "magnetization_dict"] ##Azeez added "magnetization_dict"
    def run_task(self, fw_spec):
        errors = []
        software_kwargs = deepcopy(self["software_kwargs"]) if "software_kwargs" in self.keys() else dict()
        magnetization_dict = self.get("magnetization_dict", None) ### Azeez added this magnetization_dict = self.get("magnetization_dict", None)
        ...
        try:
            if suffix == "xyz":
                ...
                sp = read(xyz)
            # Set magnetic moments before assigning calculator
            if magnetization_dict and self["software"].lower() == "espresso":       #Azeez added 
                sp = set_initial_magnetic_moments_by_element(sp, magnetization_dict)    #Azeez added 
        except Exception as e:
            if not ignore_errors:
                raise e
            else:
